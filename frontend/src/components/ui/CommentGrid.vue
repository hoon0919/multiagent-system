    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn style="margin-left: 5px;" @click="createCommentDialog = true" class="contrast-primary-text" small color="primary" >
                    <v-icon small>mdi-minus-circle-outline</v-icon>댓글 작성
                </v-btn>
                <v-dialog v-model="createCommentDialog" width="500">
                    <CreateComment
...
export default {
    ...
    methods:{
        async createComment(params){
            try{
                var path = "createComment".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, params)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','createComment 성공적으로 처리되었습니다.')
